...
# Guardamos el dataset procesado para usarlo en el informe (.qmd)
...
did_multi <- with(datos,
                  (mean(p_multi_2022[tratamiento == 1]) - mean(p_multi_2017[tratamiento == 1])) -
...
fig2 <- ggplot(datos,
               aes(x = factor(tratamiento, labels = c("Control", "Tratadas")),
...
fig3 <- ggplot(datos,
               aes(x = p_ing_2022, y = p_multi_2022,
